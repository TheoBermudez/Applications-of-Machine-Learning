...
Original file is located at
    https://colab.research.google.com/drive/1Dmkfz223m2Tg7wIh9EE8bxTXVy9ooN2-
"""
...
diabetes_knn = pd.read_csv('diabetes.csv')  # Update the filename as per your file
...
columns_to_replace = diabetes_knn.columns.drop('Pregnancies').drop('Outcome')  # Assuming 'Outcome' is your target column
...
y = diabetes_knn['Outcome'].astype('int')  # Convert to integer to ensure it's treated as categorical

# 5. Standardize the attributes of the Feature Matrix
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)
X_scaled_df = pd.DataFrame(X_scaled, columns=X.columns)  # Convert scaled data back to DataFrame to keep feature names
...
# Ensure new data has feature names for correct processing
new_data_df = pd.DataFrame(new_data, columns=X.columns)
new_data_scaled = scaler.transform(new_data_df)  # Use the scaler to transform the new data
